...
"graph neural networks" search gathered papers from 4 sources and trained a
...
1. **Search** — Dashboard → type a topic (e.g. *graph neural networks*) → *Search*.
   The system gathers papers from arXiv, Crossref, Semantic Scholar,
   and OpenAlex in parallel, scores relevance + sentiment, and shows
   results (~6–15 s).
...
- ✅ Real: live multi-source gathering, sentiment, relevance ranking, the torch
  model trained on the gathered papers, the ≥85% gate, and prediction on unseen text.
